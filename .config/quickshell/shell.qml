...
ShellRoot {
    ...
    function toggleAudio() {
        ...
        if (audioVisible) { wifiVisible = false; btVisible = false; dashboardVisible = false; musicVisible = false; launcherVisible = false; calendarVisible = false; }
    }

    function toggleCalendar() {
        calendarVisible = !calendarVisible
        if (calendarVisible) { wifiVisible = false; btVisible = false; dashboardVisible = false; musicVisible = false; launcherVisible = false; audioVisible = false; trayMenuVisible = false; }
    }

    function toggleTrayMenu() {
        trayMenuVisible = !trayMenuVisible
        if (trayMenuVisible) { wifiVisible = false; btVisible = false; dashboardVisible = false; musicVisible = false; launcherVisible = false; audioVisible = false; calendarVisible = false; }
    }
    ...
    Process {
        ...
        onExited: {
            ...
            // Workaround for layer-shell damage tracking bug under QApplication
            for (var i = 0; i < root.children.length; i++) {
                var child = root.children[i];
                if (child && child.opacity !== undefined) {
                    var oldOpacity = child.opacity;
                    child.opacity = oldOpacity - 0.001;
                    Qt.callLater(function(c, o) { 
                        if (c) c.opacity = o; 
                    }, child, oldOpacity);
                }
            }
    ...
    Process {
        id: walStepSwaync
        command: ["bash", "-c", "cp '" + root.cachePath + "/wal/colors-swaync.css' '" + root.configPath + "/../swaync/style.css' 2>/dev/null; pkill -SIGUSR1 swaync 2>/dev/null; bash '" + root.configPath + "/update-menu-style.sh'"]
        onExited: {
